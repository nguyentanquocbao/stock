...
def plot_best_ma_model(timeseries, max_orders=10, min_data_length=252):
    """
    ...
    for order in range(1, max_orders + 1):
        ...
        output = Parallel(n_jobs=-7)(
            delayed(ma_for_all_data)(timeseries, start_index, order, min_data_length)
    ...
    # MSE subplot
    plt.subplot(1, 2, 1)
    plt.plot(results["order"], results["mse"], label="MSE", color="blue")
    ...
    # MAPE subplot
    plt.subplot(1, 2, 2)
    plt.plot(results["order"], results["mape"], label="MAPE", color="red")
...
def visualize_ma(series, ma_order, min_data_length=252):
    ...
    output = Parallel(n_jobs=-7)(
        delayed(ma_for_all_data)(series, start_index, ma_order, min_data_length)
        for start_index in start_indices
    )
    fold_reals, fold_predictions = zip(*output)
    plt.figure(figsize=(10, 5))
