...
def plot_best_lag_model(timeseries, max_lags=30, min_data_length=252):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(results["code"], results["mse"], label="MSE", color="blue")
    ...
    plt.subplot(1, 2, 2)
    plt.plot(results["code"], results["mape"], label="MAPE", color="red")
    plt.title("Mean Absolute Percentage Error")
    plt.xlabel("Code")
    plt.ylabel("MAPE Value")
    plt.legend()
    plt.grid(True)
    min_mse_index = results[results["mse"] == results["mse"].min()][["mse", "code"]]
    min_mape_index = results[results["mape"] == results["mape"].min()][["mape", "code"]]
    ic(min_mse_index, min_mape_index)
    return results


def visualize(series: pd.Series, lag: int, min_data_length: int = 252):
