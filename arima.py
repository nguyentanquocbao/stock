...
def visualize_arima_results(orders, values):
    ...
    if len(orders) != len(values):
        raise ValueError("Length of orders and values must be the same")
...
def info_criteria_selection_arima(timeseries, max_order=(5, 2, 5), min_data_length=252):
    """
...
def arima_for_all_data(
    ...
    Args:
        ...
        start_index (int): Starting index for the training data.
        ma_order (List[int]): Order of the ARIMA model.
        min_data_length (int): Minimum length of the training data.

    Returns:
        pd.DataFrame: DataFrame containing the test point and the forecast.
    """
    end_index = min(len(series), start_index + min_data_length)
    train = series[start_index:end_index]
    ...
    try:
        with warnings.catch_warnings():
            ...
            model = ARIMA(train, order=arima_order, enforce_invertibility=False)
            model_fit = model.fit(method="innovations_mle")
            # Forecast
            forecast = model_fit.forecast(steps=1)
            return test_point, forecast[0]  # type: ignore
    except ValueError:
        print(f"value error order{arima_order}, start_index:{start_index}, :{end_index}")
        return None, None  # type: ignore
    except ZeroDivisionError:
        print(f"zero error order{arima_order}, start_index:{start_index}, :{end_index}")
        return None, None  # type: ignore


def plot_best_arima_model(timeseries, max_orders=(5, 2, 5), min_data_length=252, para=True):
    """
    ...
    for order in get_combo(max_orders):
        ...
        if para:
            output = Parallel(n_jobs=-7)(
                delayed(arima_for_all_data)(timeseries, start_index, order, min_data_length)
            ...
            pred = []
            for start_index in start_indices:
                real, pred = arima_for_all_data(timeseries, start_index, order, 252)  # type: ignore
            filtered_output = [
                (real, pred)
                for real, pred in [real, pred]  # type: ignore
                if real is not None and pred is not None
            ]
        ...
        try:
            result = pd.DataFrame(
                {
                    "mse": [mean_squared_error(y_true=fold_reals, y_pred=fold_predictions)],
                    "mape": [
                        mean_absolute_percentage_error(y_true=fold_reals, y_pred=fold_predictions)
                    ],
                }
            )
        except ValueError:
    ...
    min_mse_index = results[results["mse"] == results["mse"].min()][["mse", "order"]]
    min_mape_index = results[results["mape"] == results["mape"].min()][["mape", "order"]]
